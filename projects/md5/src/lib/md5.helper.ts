/**
 * Calculates MD5 from a given string
 *
...
export function md5(str: string): string {
  ...
  function strToWordArray(str: string): Array<number> {
    const len = str.length;
    const words = ((len + 8 - (len + 8) % 64) / 64 + 1) * 16;
    const result = Array<number>(words - 1);

    let count = 0;

    while (count < len) {
      const position = (count - (count % 4)) / 4;
      const shift = (count % 4) * 8;
      result[position] = result[position] | (str.charCodeAt(count++) << shift);
    }

    const position = (count - (count % 4)) / 4;
    const shift = (count % 4) * 8;
    result[position] = result[position] | (0x80 << shift);
    result[words - 2] = len << 3;
    result[words - 1] = len >>> 29;
  ...
  function numToHex(value: number): string {
    ...
    for (let idx = 0; idx < 4; idx++) {
      const byte = (value >>> (idx * 8)) & 0xFF;
      result += byte.toString(16).substr(-2).padStart(2, '0');
    }
  ...
  const x = strToWordArray(utf8Encode(str));
  ...
  return (numToHex(a) + numToHex(b) + numToHex(c) + numToHex(d)).toLowerCase();
}
